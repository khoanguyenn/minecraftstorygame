...
def Minigame(x,y,z,v,m,gameover=True): #v is the hard of the game
    ...
    if not gameover:
        mc.postToChat('Help Steve to stable the energy column.')
        time.sleep(3)
        mc.postToChat('Keep the energy level under control.')
        time.sleep(3)
        mc.postToChat('Please hold the play button')
        time.sleep(3)
    ...
    while not gameover:
        ...
        if k==y+5: # Lose condition
            ...
            return Minigame(x,y,z,v,m,False) #Restart the game
        elif p==10: # Win condition
            mc.postToChat("YOU'RE SAFE")
            gameover=True
        else:
            p+=1
Minigame(10,-5,37,0.5,0,True)
Minigame(10,-5,48,0.5,0,True)
Minigame(-10,-5,37,0.5,-1,True) 
Minigame(-10,-5,48,0.5,-1,True)
